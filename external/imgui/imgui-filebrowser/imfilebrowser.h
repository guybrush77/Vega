...
inline void ImGui::FileBrowser::Display()
{
    ...
    auto count = std::distance(pwd_.begin(), pwd_.end());
    ...
    for (auto& sec : pwd_) {
        ...
        auto prefix = secIdx == 0 ? " " : "";
        auto suffix = secIdx == (count - 1) ? "/ " : "/";

        auto padding = GetStyle().FramePadding.y;
        PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(0, padding));
        PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(0, 0));

        PushID(secIdx);
        if (secIdx > 0)
            SameLine();
        if (Button((prefix + u8StrToStr(sec.u8string()) + suffix).c_str()))
    ...
    float                 reserveHeight = GetFrameHeightWithSpacing() + 15;
    std::filesystem::path newPwd;
        ...
        reserveHeight += GetFrameHeightWithSpacing();
    {
        ...
        for (auto& rsc : fileRecords_) {
            ...
            if (rsc.isDir) {
                PushStyleColor(ImGuiCol_Text, ImVec4(0.3f, 0.57f, 1.0f, 1.0f));
            } else {
...
inline void ImGui::FileBrowser::SetPwdUncatched(const std::filesystem::path& pwd)
{
    ...
    for (auto& p : std::filesystem::directory_iterator(pwd)) {
        ...
        rcd.showName = u8StrToStr(p.path().filename().u8string()) + (rcd.isDir ? "/" : "");
